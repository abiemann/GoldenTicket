...
The optional argument is an output directory. The runner creates PNG screenshots at 1280×800 and 1000×620, with an additional 875×680 five-character roster render, including bottom-of-scroll views where relevant, plus `layout-report.json` and `binding-errors.log`. It fails for WPF binding warnings/errors, missing resources, interactive controls overflowing the horizontal viewport, or primary button labels overriding their intended foreground. It also sends synthetic WPF keyboard routed events directly to the camera preview's key handler, without injecting operating-system input. These checks exercise initial corner placement, selecting and adjusting existing corners before and after initial selection, returning to placement, image-edge clamping, and keeping photo capture disabled without a fresh camera frame. The production pointer-coordinate mapper is checked against the displayed image bounds and letterboxing. Results are written to `camera-corner-interactions.json`.

Separate synthetic solo and shared-human fixtures exercise setup guidance, the phone command's availability, automatic solo opening destinations and turn cards, the opening choice's lack of **Back to table** and plain-Escape dismissal, later **Your cards** / **Back to table** labels, deliberate covering, and explicit handoffs between two humans. Technical and system recovery paths remain available. The private-view checks require all hand and destination controls to bind to the currently revealed human's own collections. Each view is rendered at both sizes, and results are written to `human-presentation-interactions.json`. These checks complement the domain/privacy regression suite; they do not claim that inspecting a visual tree proves every hidden-information boundary.

The solo fixture also checks the confirmed-drop transition: the rejected card and its board connection vanish together, the remaining cards panel starts sliding down, and the draw piles and face-up market slide to the centered two-player layout after the public board returns. Static table fixtures cover the centered two-to-four-player layout and the outer five-player layout. Five-player renders also open train-card and destination trays for every seat at both window sizes, checking that trays avoid other tiles, following left seats move when needed, right seats stay in their usual positions, and closing restores the layout. Player 4's taller destination tray fits above that player's fixed bottom-right tile. The human tile then opens the kept private destinations.
...
Camera processing fixtures verify the default best-native capture request and Auto processor choice,
original-frame preview labeling, and the absence of the removed Enhanced preview control, Piece
outlines panel, score cards, overlay, and Reload ML model control. The preview and editable corner
handles remain available at
both viewport sizes. A separate `--marker-scores` mode checks internal synthetic score publication
for valid, missing, duplicate, unknown, and off-track readings without displaying those diagnostics
in Camera. Actual detector tests and `GoldenTicket.PieceDetectionSmoke` check recognition separately.
