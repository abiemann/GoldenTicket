...
camera. A September 23 physical retest after the identity-preserving fix reported that the game
re-found and displayed the webcam stream after reconnect; see section 24 for the tested scope.
...
| Packaging and local install | The v1.0.0 per-user [installer](docs/windows-installer.md) and self-contained ZIP were built from the release source commit. On September 23, the user reported installing Setup on the development Windows 11 25H2 x64 PC (build 26200.9457), launching GoldenTicket, and starting a game. | The installer completed and the installed app reached gameplay on that PC. This does not establish a clean-machine install, a completed match from the installed build, webcam use in that session, or uninstall/save-retention behavior. |
...
test and browser/render checks. Failed runs attempt to retain evidence when GitHub artifact
storage is available; the workflow does not publish a release.
...
| Post-claim scoring marker after restart | The marker obligation survives SQLite restoration and the desktop restores its guidance in automated tests. Physically restart after a committed claim but before moving its scoring marker; confirm the next action stays blocked until a fresh camera check accepts the move. |
| Clean-machine distribution | The September 23 development-PC install, launch and game start passed. Run the v1.0.0 distribution offline on a clean Windows 11 machine with its bundled dependencies. Verify installer uninstall and saved-game retention; upgrade needs a later version. |
