...
public class PersistenceTests : IDisposable
{
    ...
    public async Task AMatchSurvivesBeingClosedAndReopened()
    {
        ...
        var coordinator = await GameCoordinator.CreateAsync(
            rules, store, Setup(), DeterministicRandom.SeedFrom(123), TestContext.Current.CancellationToken);
        ...
        var beforeHash = await coordinator.ComputeStateHashAsync(TestContext.Current.CancellationToken);
        var beforeVersion = coordinator.Public.StateVersion;

        // Reopen from disk exactly as a restart would.
        var reopened = await GameCoordinator.RestoreAsync(rules, store, coordinator.SessionId, TestContext.Current.CancellationToken);

        Assert.Equal(beforeHash, await reopened.ComputeStateHashAsync(TestContext.Current.CancellationToken));
    ...
    public async Task RepeatingACommandIdReturnsTheFirstOutcomeWithoutApplyingItTwice()
    {
        ...
        var coordinator = await GameCoordinator.CreateAsync(
            rules, store, Setup(2), DeterministicRandom.SeedFrom(321), TestContext.Current.CancellationToken);
        ...
        var first = await coordinator.SubmitAsync(command, TestContext.Current.CancellationToken);
        Assert.True(first.IsAccepted);
        Assert.False(first.WasDuplicate);

        var versionAfterFirst = coordinator.Public.StateVersion;
        var hashAfterFirst = await coordinator.ComputeStateHashAsync(TestContext.Current.CancellationToken);

        var second = await coordinator.SubmitAsync(command, TestContext.Current.CancellationToken);

        Assert.True(second.WasDuplicate);
        Assert.Equal(versionAfterFirst, coordinator.Public.StateVersion);
        Assert.Equal(hashAfterFirst, await coordinator.ComputeStateHashAsync(TestContext.Current.CancellationToken));
    ...
    public async Task ARejectedCommandIsRememberedSoARetryGetsTheSameAnswer()
    {
        ...
        var coordinator = await GameCoordinator.CreateAsync(
            rules, store, Setup(2), DeterministicRandom.SeedFrom(77), TestContext.Current.CancellationToken);
    ...
    public async Task ASavedMatchIsListedWithoutRevealingAnything()
    {
        ...
        await GameCoordinator.CreateAsync(rules, store, setup, DeterministicRandom.SeedFrom(9), TestContext.Current.CancellationToken);

        var sessions = await store.ListSessionsAsync(TestContext.Current.CancellationToken);
    ...
    public async Task PrivatePayloadsAreReadableInTheDatabaseAndStillRestore()
    {
        ...
        var coordinator = await GameCoordinator.CreateAsync(
            rules, store, Setup(2), DeterministicRandom.SeedFrom(4242), TestContext.Current.CancellationToken);
        ...
        await using (var sessionCommand = connection.CreateCommand())
        {
            ...
            Assert.Equal(SqliteSessionStore.StoreSchemaVersion,
                Convert.ToInt32(await sessionCommand.ExecuteScalarAsync(TestContext.Current.CancellationToken)));
        }
        ...
        Assert.Equal(await coordinator.ComputeStateHashAsync(TestContext.Current.CancellationToken),
            await reopened.ComputeStateHashAsync(TestContext.Current.CancellationToken));
    }
    ...
    public async Task ATamperedJournalRowIsDetectedOnRestore()
    {
        ...
        var coordinator = await GameCoordinator.CreateAsync(
            rules, store, Setup(2), DeterministicRandom.SeedFrom(31), TestContext.Current.CancellationToken);
        ...
        await Assert.ThrowsAsync<SessionIntegrityException>(
            () => GameCoordinator.RestoreAsync(rules, store, sessionId, TestContext.Current.CancellationToken));
    }
}
